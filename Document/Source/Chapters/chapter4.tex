...
\subfigure[Ground Truth Segmentation]{
   \includegraphics[width=0.31\linewidth] {implementation/merger/mask}
   \label{fig:merger_result_a}
}
\subfigure[Generated Segmentation]{
   \includegraphics[width=0.31\linewidth] {implementation/merger/oversegmentation}
   \label{fig:merger_result_b}
}
\subfigure[Merged Segmentation]{
   \includegraphics[width=0.31\linewidth] {implementation/merger/merged}
...
\end{figure}
Usually, our results exhibit an oversegmentation as illustrated in figure $\ref{fig:merger_result_b}$. Therefore we want to apply a merging stage onto our results before running the evaluation. The goal of the merging phase is to lower the number of total segments but the same time retain meaningful segmentation labels. Conceptually, our merging approach is visualized in figure $\ref{fig:merger_result}$. Different approaches how to implement such a merging mechanism are described in literature (LIST SOME REFS HERE). However, we use a much simpler approach. For every resulting segment we determine is closest corresponding ground truth mask label and assign it to it. \\ \\
Basically, a segmentation $S$ is as a set of points in which every point is assigned to a certain segment identifier. In the following let us denote $S_j$ as the subset of points in $S$ that belong to the segmentation label $j$. Similarly, let $M$ denote the set of all ground truth points with their corresponding mask labels and $M_i$ the set of all points that belong to the ground truth mask $i$. Then, to find the corresponding ground truth label for a segment $S_j$ we simply only have to evaluate formula given by equation $\ref{eq:merging_label_formula}$. 
\begin{equation}
i = \argmaxl_{M_i \in M} \left\vert{S_j \cap M_i}\right\vert
\label{eq:merging_label_formula}
\end{equation}
After applying this equation to every segment $S_j$ we have computed the merged segmentation version of the initial oversegmentation. An example is shown in figure $\ref{fig:merger_result_c}$.
