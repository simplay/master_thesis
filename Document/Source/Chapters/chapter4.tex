...
In this section we describe how we compute the so called affinity matrix, which is later used to compute the actual motion segmentation. Such a matrix basically encodes a similarity value between trajectories according to a predefined measure.

Hence, as the very first step our pipeline fetches the previously extracted trajectories to form every possible pair combination.
For each trajectory pair different distance values are computed and combined according to an initially by a pre-specified run-mode.
There are two main setting a user has to specify preliminarily: Whether or not depth cues should be used and which kind of similarity measure should be used. All possible modes are listed in figure $\ref{fig:affinity_modes}$.
\begin{figure}[H]
\begin{center}
