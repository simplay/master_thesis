...
In this section we describe how we compute the so called affinity matrix, which is later used to compute the actual motion segmentation. Such a matrix basically encodes a similarity value between trajectories according to a predefined measure. Hence, as the very first step our pipeline fetches the previously extracted trajectories to form every possible pair combination. For each trajectory pair different distance values are computed and combined according to an initially by a pre-specified run-mode. More precisely, our pipeline computes the following three distances between overlapping trajectory segments: The spatial distance $d_{\text{spatial}}$, the color distance $d_{\text{color}}$ and the motion distance $d_{\text{motion}}$. \\ \\
