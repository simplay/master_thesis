...
In this last experiment we perform an overall evaluation of our main methods using their standard parameter setup. For this evaluation we use five datasets (cerealbox, chairs, watercan, waving arm, statue). Each method computes the segmentation on each dataset. For each dataset we use 3 ground truth images, which we use to evaluate the quality of the resulting segmentations. The averaged performance is listed in table \ref{tab:overall_performance}.
