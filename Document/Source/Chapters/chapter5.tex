...
So far, we justified our default parameter selections and were able to drop two poorly behaving flow methods (HS and LRGBD). In this experiment, we want to evaluate the segmentation quality of all LDOF and SRSF pipeline combinations applied on five datasets (Cerealbox, Chairs, Watercan, Waving-Arm and Statue). The reason for not using every dataset is because not every method can be run on every available dataset. For instance, SRSF flow fields can only be generated for images with a resolution of $640 \times 480$ pixels. Furthermore, we do not use datasets that do not have depth fields.\\ \\
...
Additionally, we evaluated all LDOF P-affinity pipeline combinations on every dataset. The results are listed in Table $\ref{tab:overall_performance_ldof}$.
\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
...
Additional experiments and results can be found in the appendix Chapter $\ref{chap:additional_exp}$.
