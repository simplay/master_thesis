...
\section{Methodology}
run different pipeline component combinations on our datasets. According to the run experiment, different pipeline and parameter settings are used. An exact description is given initially stated in the corresponding experiment. To reduce the effect of oversegmentation we post-process the produced motion segmentations via our segment merger. Finally, we qualitatively evaluate the quality of the merged segments by computing different statistical measures. By comparing the segments against their ground truth images, we compute the precision, recall, the f1 score and the fragmentation. 
% under construction
