...
class MatlabScript
  ...
  def initialize(path_to_script, script_name, cmd_before = "")
    @path_to_script = path_to_script.empty? ? Dir.pwd + "/" : path_to_script
  ...
  def execute(arg_container, prepended_commands = "")
    args = serialize_args(arg_container)
    run_matlab = <<-SCRIPT
      matlab -nosplash -nodesktop -r \"run ${PWD}/#{@script_name}(#{args}) ; exit\"
    SCRIPT
    cmd = "cd #{@path_to_script} && " + run_matlab.lstrip
    cmd = "#{@cmd_before} && #{cmd}" if cmd_before?
    system(cmd)
  end
