...
public class Point2d {
    ...
    // This flag is used to filter helper tracking points,
    // to have similarity values between shortly cut trajectories.
    private boolean belongsToAddition = false;

    // Euclidean position is space.
     ...
     *
     * Addition points are post-continued tracking points (either appended or prepended).
     * We make use of such points to allow to compute similarities between shortly cut
     * trajectories. However, keep in mind, that this trajectory extension points were
     * not tracked by making use of optical flow cues. Therefore, it makes no sense to
     * render these points. This getter allows to determine whether we have to filter a point,
     * because it is a fake tracking point, used for auxiliary purposes for computing affinities
     * between non-overlapping trajectories.
     *
     ...
     * activities.
     *
     * Only mark trajectory points generated by calling Trajectory#extendPointTracking()
     * Since those have no render-significance.
     */
