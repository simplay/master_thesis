function run_init_data( dataset, sampling_rate, compute_tracking_data, compute_flow_variances, compute_cie_lab, extract_depth_fields, compute_depth_variances )
%RUN_INIT_DATA Summary of this function goes here
%   Detailed explanation goes here
    ...
    COMPUTE_TRACKING_DATA = compute_tracking_data; % compute tracking candidates, valid regions, flows
    COMPUTE_FLOW_VARIANCES = compute_flow_variances; % compute local and global flow variance
    COMPUTE_CIE_LAB = compute_cie_lab; % compute cie lab colors from given input seq
    EXTRACT_DEPTH_FIELDS = extract_depth_fields; % add check: only if depth fields do exist
    COMPUTE_DEPTH_VARIANCE = compute_depth_variances;
    ...
    if EXTRACT_DEPTH_FIELDS
        ...
        for k=1:length(imgs)
            ...
            if COMPUTE_DEPTH_VARIANCE
                ...
                % IMPORTANT: DEPTH FIELD VALUES HAVE TO BE IN METER SCALE
                [vard, ~] = computeLocalDepthVar(lv, VAR_SIGMA_S, 0.1, lv > 0);
                ...
                var_img = vard ./ max(vard(:));
                imwrite(var_img, imgfile);
                fid = fopen(fname,'w');
                if fid ~= -1
                    for t=1:size(vard,1)
                        a_row = mat2str(vard(t,:));
                        fprintf(fid,'%s\r\n', a_row);
                    end
                end
                fclose(fid);
    ...
    if COMPUTE_FLOW_VARIANCES
        ...
        for t=START_FRAME_IDX:END_FRAME_IDX
            ...
            flow_k = fw_flow(:);
            flow_variance = var(flow_k);
            global_variances = [global_variances, flow_variance];
        end
